...
async def show_start(chat_id, first_name, user_id):
    ...
    await send_message(chat_id,
        ...
        "Если ты уже оплатила, нажми «🆔 Узнать мой ID», скопируй цифры и отправь их Лё. "
        "После того как Лё добавит тебя, нажми «🔄 Проверить доступ» и курс откроется.",
...
async def send_course(chat_id):
    ...
    await send_message(chat_id,
        "В праславянском языке слово «ведьма» не несло никакого тёмного смысла. Оно буквально означало «ведающая» - та, кто знает. "
        ...
        "Женщина-творец - это та, кто создаёт реальность из этой глубины. Она не подстраивается под мир, она формирует "
        ...
        "В этом видео разбор хореографии, которая воплощает именно этот образ. Как через тело войти в роль женщины, "
        "которая создаёт свою реальность."
    )
    await asyncio.sleep(1)
    ...
    await send_video(chat_id,
        ...
        "Это и есть техника раскрытия БАТ, самая неожиданная и самая мощная форма защиты. Полное "
        ...
        "Когда точки открыты, ты максимально восприимчива к себе, к партнёру. К тому, что хочет прийти через тебя.\n\n"
        "В этом видео практика, которая помогает это сделать."
    )
    await asyncio.sleep(1)
...
async def send_bonus(chat_id):
    await send_message(chat_id,
        ...
        "Они знали: настоящее обольщение - это ритуал. Энергия, намерение, преображение, когда ты не просто приходишь к мужчине - "
        "а являешься ему. Как богиня. Как тайна. Как то, что он не может объяснить, но не может забыть.\n\n"
        ...
        "Все великие трактаты о любви говорят одно: занятие любовью между мужчиной и женщиной - это божественное мероприятие. "
        "И когда оно наполнено алхимией, волшебством и экстазом - это уже не просто близость. а священный ритуал двоих.\n\n"
    ...
    if text == "🔄 Проверить доступ":
        if user_id in allowed_users or user_id in ADMIN_IDS:
            ...
            await send_message(chat_id,
                ...
                "Убедись, что отправила свой ID Лё. Как только она добавит - нажми кнопку снова."
            )
        return
