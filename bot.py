...
VIDEOS = [
    # {"title": "Урок 1", "file_id": "BAACAgI...", "text": ""},
]
...
async def send_message(chat_id, text):
    return await api("sendMessage", chat_id=chat_id, text=text, parse_mode="Markdown")
...
async def handle_update(update):
    ...
    if text == "/start":
        ...
        try:
            ...
            await send_message(target_id, "🎉 Оплата получена! Напиши /start чтобы получить курс")
...
async def send_course(chat_id):
    await send_message(chat_id, "✨ Доступ открыт! Отправляю материалы курса...\n\nСохрани этот чат 🙏")
    if not VIDEOS:
        await send_message(chat_id, "⏳ Видео скоро будут здесь!")
        return
    for video in VIDEOS:
